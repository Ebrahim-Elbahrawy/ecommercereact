...
const ProductCardHook = (items, favProd) => {
  ...
  const resAdd = useSelector((state) => state.wishListReducer.addFavourite);
  const resRemove = useSelector(
    (state) => state.wishListReducer.removeFavourite
  );
  ...
  const removeToWishListData = async () => {
    ...
    await dispatch(removeProductFromWishList(items._id));
